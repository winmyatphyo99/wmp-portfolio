  ...
  <section id="projects" class="relative py-32 bg-[#050505] border-t border-white/5 overflow-hidden">
    ...
    <div class="container-system relative z-10 px-6 mx-auto max-w-7xl">
      ...
      <div class="mb-16 space-y-10">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <button @click="clearAll" v-show="isFiltered"
            ...
            <svg class="w-3 h-3 text-cyan-400 group-hover:text-black" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
        ...
        <div class="grid lg:grid-cols-12 rounded-3xl border border-white/5 bg-[#0a0a0a] overflow-hidden shadow-2xl">
          ...
          <div class="lg:col-span-4 bg-black/40 border-l border-white/5 p-3">
            <div class="relative">
              ...
              <svg class="absolute left-4 top-3.5 text-white/20" width="18" height="18" fill="none"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 relative">
        <div v-if="filteredProjects.length === 0"
          class="col-span-full py-32 text-center rounded-3xl border border-dashed border-white/10 bg-white/[0.01]">
          <p class="text-white/40 text-sm font-medium tracking-wide">No production systems match your query.</p>
          <button @click="clearAll"
            class="mt-4 text-cyan-400 text-xs font-black uppercase tracking-widest hover:text-white transition">
            Clear all filters
          </button>
        </div>

        <div v-for="(proj, idx) in filteredProjects" :key="proj.title" @click="openDetails(proj)"
          class="group relative flex flex-col cursor-pointer transition-all duration-500" v-motion
          :initial="{ opacity: 0, y: 40 }" :visibleOnce="{
            opacity: 1,
            y: 0,
            transition: {
              delay: idx * 80
          ...
          <div class="relative aspect-[16/10] rounded-[2rem] overflow-hidden border border-white/5 bg-[#0a0a0a]">
            <!-- <img :src="proj.image" loading="lazy" decoding="async" class="w-full h-full object-cover" /> -->
            <picture>
              <source :srcset="proj.imageWebp" type="image/webp" />
              <img :src="proj.image" loading="lazy" decoding="async"
                class="w-full h-full object-cover opacity-50 group-hover:scale-110 transition-all duration-1000" />
            </picture>
            <!-- Overlay Info -->
            <div class="absolute inset-0 bg-gradient-to-t from-[#050505] via-transparent to-transparent opacity-80">
            </div>
            ...
            <div class="absolute bottom-8 left-8 right-8">
              <span class="text-[10px] font-black text-cyan-500/60 uppercase tracking-[0.4em] mb-2 block">System 0{{ idx
                + 1 }}</span>
    ...
    <Transition name="modal">
      <div v-if="selectedProject"
        ...
        <div
          class="w-full max-w-6xl bg-[#050505] border border-white/10 rounded-[2.5rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] relative flex flex-col md:max-h-[90vh] overflow-hidden">

          <div class="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
            <span class="text-[10px] font-black text-cyan-500 uppercase tracking-[0.3em]">Technical Deep-Dive</span>
            <button @click="closeDetails"
              class="p-2 rounded-full hover:bg-white/10 text-white/40 hover:text-white transition">
          ...
          <div class="overflow-y-auto custom-scrollbar p-8 md:p-16">
            <div class="grid lg:grid-cols-2 gap-16">
                ...
                <div class="space-y-6">
                  ...
                  <div class="grid gap-4">
                    <div v-for="feat in selectedProject.features" :key="feat"
                      ...
                      <div
                        class="mt-1.5 h-1.5 w-1.5 rounded-full bg-cyan-500 flex-shrink-0 shadow-[0_0_10px_rgba(6,182,212,0.8)]">
                      </div>
              ...
              <div class="space-y-10">
                ...
                <div
                  class="p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-4 relative overflow-hidden">
