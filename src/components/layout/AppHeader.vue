  ...
  <header class="fixed top-0 w-full z-50 bg-[#05070A]/80 backdrop-blur-md border-b border-white/5">
    ...
    <div class="container-system h-20 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3 cursor-pointer group" @click="go('hero')">
        <div class="w-10 h-10 grid place-items-center rounded-xl bg-purple-600 text-white font-bold text-sm shadow-lg shadow-purple-500/20 group-hover:bg-purple-500 transition-colors">
      ...
      <nav class="hidden md:flex items-center gap-10">
        ...
        >
          {{ s.label }}
          <span v-if="activeSection === s.id" class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-purple-400"></span>
        </button>
      </nav>
    ...
    <transition name="fade">
      <div v-if="isOpen" class="fixed inset-0 top-20 bg-[#05070A] p-8 md:hidden z-40">
        <div class="flex flex-col space-y-8 mt-10">
          <button
            v-for="s in sections"
            :key="s.id"
            @click="go(s.id)"
            class="text-4xl font-bold tracking-tighter text-left transition-colors"
